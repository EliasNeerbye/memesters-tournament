...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({
        message: "Code didn't get generated, or email didn't get sent.",
      });
  } catch (error) {
    ...
    return res
      .status(500)
      .json({
        message: "Failed to generate verification code, or email was not sent",
      });
  }
});
...
router.get("/profile", async (req, res) => {
  ...
  if (result.error) {
    return res
      .status(401)
      .json({ message: "Authentication failed", error: result.error });
  }
...
router.put("/profile/username", async (req, res) => {
  ...
  if (typeof username !== "string") {
    return res
      .status(400)
      .json({ message: "Username needs to contain letters!" });
  }

  if (username.length < 3 || username.length > 16) {
    return res
      .status(400)
      .json({ message: "Keep name between 3 and 16 characters long!" });
  }
  ...
  if (!/[a-zA-Z]/.test(username)) {
    return res
      .status(400)
      .json({ message: "Username must contain at least one letter!" });
  }
...
router.put("/profile/email", async (req, res) => {
  ...
  if (typeof code !== "string" || !/^\d{6}$/.test(code)) {
    return res
      .status(400)
      .json({ message: "Code needs to be a 6-digit number!" });
  }
  ...
  const alreadyExists = User.findOne({
    $or: [{ email: newEmail }, { tempEmail: newEmail }],
  });
  ...
  try {
    ...
    if (result) {
      ...
      await User.findOneAndUpdate(
        { email: user.email },
        { tempEmail: newEmail, isVerified: false }
      );
      return res
        .status(201)
        .json({ message: "Sent verification code to new email." });
    }
  } catch (error) {
    console.error("Login error:", error);
    return res.status(400).json({ message: error.message });
...
router.post("/profile/email/code", async (req, res) => {
  ...
  try {
    ...
    if (result) {
      ...
      return res
        .status(201)
        .json({ message: "Email has been updated!", token: newToken });
    } else {
...
router.put("/profile/pfp", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Profile picture updated successfully", pfpUrl });
  } catch (error) {
...
router.get("/isLoggedIn", async (req, res) => {
  ...
  const result = await AuthService.jwtValidation(token, secret);
  if (result._id) {
    return res.status(201).json({ message: "User is logged in!" });
  } else {
